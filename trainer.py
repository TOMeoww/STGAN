...
class STGANtrainer(nn.Module):
    ...
    def gen_update(self, attr_fake, attr_real, imgs_real, epoch):
        ...
        recon_loss = self.compute_recon_loss(imgs_recon,imgs_real)* self.hyperparameters['lambda3']

        #attr_loss
        imgs_fake = self.gen(imgs_real, attr_fake, attr_real)
        attr, adv = self.dis(imgs_fake)
        attr_loss = self.compute_attr_loss(attr, attr_fake, mode = 'gen')* self.hyperparameters['lambda2']

        #adv loss
        adv_loss = self.compute_adv_loss(adv, mode = 'gen')

        gen_total_loss = recon_loss  +  attr_loss  + adv_loss
        
        gen_total_loss.backward(retain_graph=True)

        self.gen_opt.step()  
                                               
        return imgs_fake,gen_total_loss,recon_loss,attr_loss,adv_loss
    ...
    def dis_update(self, imgs_real,attr_fake, attr_target_real, epoch, alpha, fake):
        ...
        #adv_loss
        adv_loss = self.compute_adv_loss(fake_validity, real_validity, mode = 'dis')
        attr_loss = self.hyperparameters['lambda1'] * self.compute_attr_loss(attr, attr_target_real, mode = 'dis')
